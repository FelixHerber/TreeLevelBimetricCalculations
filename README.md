...
The tree-level scattering amplitudes of bimetric theory. 
...
`ExampleCalculationsBimetricAmplitudes.nb`. The required model files and
intermediate expansions have already been generated. 
...
### frForBimetric.nb
The notebook that uses FeynRules to read xActExpansionOfBimetricTheory.fr and then generates the FeynArts model files that can be found in the xActExpansionOfBimetricTheory_FA folder. Some checks are also applied in this notebook, such as checking whether the Lagrangian is Hermitian and whether it is diagonal in the mass eigenstates. Note that the file directories here reference files on my computer, so if anyone else wants to use it, they must change these directories to corresponding ones on their computer.

The notebook contains absolute paths from the original development environment. Before running it on another system, replace the paths to the FeynRules installation and the .fr model file with the corresponding local paths.
...
Contains a list of all 100 distinct 2->2 helicity amplitudes of bimetric theory, where distinct means distinct with respect to the symmetries of crossing, parity and particle exchange (swap symmetry). 
...
Computes all 100 distinct tree-level helicity amplitudes using the functions in BimetricAmplitudes.wl on an HPC. 
...
### GetOrbitsBinAmps.nb
This notebook generates representatives of the 100 distinct orbits of the group \(G_{2\to2}\). That group is generated by crossing symmetry, parity and particle exchange (swap). See section 7.2 in the thesis (F. Herber (2025)) for an explanation. 
...
This repository was developed as part of a Master’s degree project.
Because my current research concerns other topics, active maintenance and support may be limited.
